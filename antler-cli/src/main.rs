...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Fetch {
            ...
            fetch_command(&artifact, output, sources, javadoc)?;
        }
...
"#,
                ...
                artifact.sha1.as_deref().unwrap_or("TODO"),
...
fn fetch_command(coord: &str, _output: PathBuf, _sources: bool, _javadoc: bool) -> Result<()> {
    let artifact = Artifact::parse(coord)?;
    info!("Fetching {}...", artifact);
    // TODO: Implement actual fetching
    println!("Fetch not yet implemented");
    Ok(())
}
