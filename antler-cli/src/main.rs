...
use antler_core::{Artifact, Repository, Resolver};
...
async fn resolve_command(
    ...
    for (i, url) in extra_repos.iter().enumerate() {
        resolver = resolver.with_repository(Repository::custom(
            format!("custom-{i}"),
...
fn generate_buck_output(resolutions: &[antler_core::Resolution]) -> String {
    let mut output = String::new();
...
async fn fetch_command(coord: &str, output: PathBuf, sources: bool, javadoc: bool) -> Result<()> {
    ...
    // Use the resolver to get checksums and find the artifact
    ...
    download_and_verify(&jar_url, &jar_path, resolved.sha256.as_deref(), resolved.sha1.as_deref()).await?;
    info!("Downloaded {}", jar_path.display());

    // Optionally download sources
    if sources {
        let sources_artifact = Artifact {
