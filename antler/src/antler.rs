...
//!
//! This module provides [`Antler`], the main entry point for resolving
//! JVM dependencies using a fluent builder pattern.

use std::path::Path;

use jvm_artifact::Artifact;
use jvm_fetch::{Fetcher, MavenRepository, RepositoryList};
use jvm_resolver::{Resolution, ResolvedArtifact, ResolverConfig};
...
///
/// `Antler` provides a fluent builder API for configuring and executing
...
///
/// ```no_run
/// use antler::{Antler, ResolverConfig};
///
/// let antler = Antler::new()
///     .with_maven_central()
///     .with_google()
///     .transitive(true)
...
impl Antler {
    /// Creates a new `Antler` instance with default settings.
    ///
    ...
    /// Creates a new `Antler` instance with default repositories (Maven Central + Google).
    ///
    ...
    ///
    /// ```no_run
    /// use antler::{Antler, MavenRepository};
    ///
    /// let antler = Antler::new()
    ...
    ///
    /// # async fn example() -> antler::Result<()> {
    /// let resolution = Antler::with_defaults()
    ...
    pub async fn resolve(&self, artifact: &Artifact) -> Result<Resolution> {
        ...
        let sha1 = fetcher
            .fetch_checksum(artifact, jvm_fetch::ChecksumAlgo::Sha1)
            .await
            .ok()
            .flatten();
        let sha256 = fetcher
            .fetch_checksum(artifact, jvm_fetch::ChecksumAlgo::Sha256)
            .await
            .ok()
            .flatten();
        ...
        if self.config.transitive {
            for dep in pom.direct_dependencies() {
                ...
                if let Some(version) = pom.resolve_dependency_version(dep) {
                    ...
                    let sha1 = fetcher
                        .fetch_checksum(&dep_artifact, jvm_fetch::ChecksumAlgo::Sha1)
                        .await
                        .ok()
                        .flatten();
                    let sha256 = fetcher
                        .fetch_checksum(&dep_artifact, jvm_fetch::ChecksumAlgo::Sha256)
                        .await
                        .ok()
                        .flatten();
...
mod tests {
    ...
    fn test_antler_with_defaults() {
        let antler = Antler::with_defaults();
