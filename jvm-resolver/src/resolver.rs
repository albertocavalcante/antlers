...
use jvm_artifact::{Artifact, Exclusions, Project, Version, VersionConstraint};
...
///
/// ```ignore
/// use jvm_resolver::{Resolver, ResolverConfig, NearestWins};
///
...
mod tests {
    ...
    struct MockProject {
        artifact: Artifact,
        deps: Vec<jvm_artifact::Dependency>,
    }
    ...
    impl Project for MockProject {
        ...
        fn dependencies(&self) -> Vec<&jvm_artifact::Dependency> {
            self.deps.iter().collect()
        }

        fn managed_dependencies(&self) -> Vec<&jvm_artifact::ManagedDependency> {
            Vec::new()
        }

        fn parent(&self) -> Option<&jvm_artifact::ParentRef> {
            None
        }
    ...
    async fn test_resolve_with_dependency() {
        ...
        let dep = jvm_artifact::Dependency::new(Coordinates::new("com.example", "dep"))
    ...
    async fn test_conflict_resolution_nearest_wins() {
        ...
        let b_dep_1 = jvm_artifact::Dependency::new(Coordinates::new("com.example", "b"))
            .with_version(VersionConstraint::Exact(Version::new("1.0")));
        let c_dep = jvm_artifact::Dependency::new(Coordinates::new("com.example", "c"))
            .with_version(VersionConstraint::Exact(Version::new("1.0")));
        ...
        let b_dep_2 = jvm_artifact::Dependency::new(Coordinates::new("com.example", "b"))
